...
//(username and password) and being able to (when logged in) access newForum
// page to insert new book forum 
//being able to see new forums on home page and click on details about them, inside detail page
//being able to insert new comment about that topic
...
Deno.test('login with wrong username', async test =>{
    //GIVEN I am on the home page and I have accepted All Cookies
        const args = ['--window-size=800,1000']
    ...
    //AND I click on Log In button
        await page.click('a[href="/login"]', { waitUntil: 'networkidle0' })
    //WHEN I insert wrong username 'test' in username field
    ...
    //AND I press Enter
        await page.keyboard.press('Enter')
        await page.waitForTimeout(1000)
    //THEN I should be still on Log In page
        const heading = await page.$eval('h1', node => node.innerText)
		await assertEquals(heading, 'Log In', 'logging in does not take user to Home page')
        await browser.close()
})
Deno.test('login with wrong password', async test =>{
    //GIVEN I am on the home page and I have accepted All Cookies
        const args = ['--window-size=800,1000']
        const browser = await puppeteer.launch({ headless: true, args })
        const page = (await browser.pages())[0]
    ...
    //WHEN I insert username 'doej' in username field
        await page.type('input[name="username"]', 'doej')
    //AND I insert wrong password 'p455w0rd2' in password field
        await page.type('input[name="password"]', 'p455w0rd2')
    //AND I press Enter
        await page.keyboard.press('Enter')
        await page.waitForTimeout(1000)
    //THEN I should be still on Log In page
...
Deno.test('testing insertion comments to the forum topics  ', async test => {
    //GIVEN I am on home page and I have accepted All Cookies
        const browser = await puppeteer.launch({ headless: false })
        const page = (await browser.pages())[0]
        await page.goto('https://winter-sabine-8080.codio-box.uk/', { waitUntil: 'networkidle0' })
    ...
    //WHEN I click on Show Forum button
      await page.click('a[href="/details/1"]', { waitUntil: 'networkidle0' })
      await page.waitForTimeout(5000)
      await page.type('textarea[name="comment"]', 'Really nice topic')
      await page.waitForTimeout(1000)
      await page.keyboard.press('Enter')
      await page.waitForTimeout(1000)
      await browser.close()
